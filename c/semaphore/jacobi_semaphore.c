...
void *worker_thread(void *arg) {
  ...
  for (int step = 1; step <= args->steps; step++) {
    ...
    if (args->is_upper) {
      ...
      memcpy(args->my_boundary_writer, &src[(rows - 1) * M],
             M * sizeof(double));
    } else {
      // Lower: 先頭の行 (0) を書き込む
    ...
    while (atomic_load_explicit(args->other_ready, memory_order_acquire) <
           (size_t)step) {
      CPU_RELAX();
    }
    ...
    for (int i = 1; i < rows - 1; i++) {
      for (int j = 1; j < M - 1; j++) {
        int idx = i * M + j;
        double laplacian = src[idx - M] + src[idx + M] + src[idx - 1] +
                           src[idx + 1] - 4.0 * src[idx];
    ...
    if (args->is_upper) {
      ...
      for (int j = 1; j < M - 1; j++) {
        int idx = i * M + j;
        double down_val =
            args->other_boundary_reader[j]; // 共有バッファから読み取り
        double laplacian = src[idx - M] + down_val + src[idx - 1] +
                           src[idx + 1] - 4.0 * src[idx];
        dst[idx] = src[idx] + factor * laplacian;
      }
    } else {
      ...
      for (int j = 1; j < M - 1; j++) {
        ...
        double up_val =
            args->other_boundary_reader[j]; // 共有バッファから読み取り
        double laplacian = up_val + src[idx + M] + src[idx - 1] + src[idx + 1] -
                           4.0 * src[idx];
        dst[idx] = src[idx] + factor * laplacian;
      }
    }
    ...
    while (atomic_load_explicit(args->other_done, memory_order_acquire) <
           (size_t)step) {
      CPU_RELAX();
    }
...
void run_safe_semaphore_optimized(Grid *a, Grid *b, int steps) {
  ...
  double *boundary_upper =
      (double *)calloc(M, sizeof(double)); // Upperが書き込む
  double *boundary_lower =
      (double *)calloc(M, sizeof(double)); // Lowerが書き込む
